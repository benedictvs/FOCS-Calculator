...
class mispell_combinations_model(solver_model):
    ...
    def misspell(self, n: str, k: int) -> str:
        ...
        # standard misspell
            ...
            return "For the word {} there are {} misspellings\n".format(
                s, str(self.factorial(length) / fact - 1)
            )
        # standard permutation formula
            ...
            if k > length:
                ...
                return "For the word {} there are {} substrings".format(
                    s, factlen / denom
                ) + " which are of length {}\n".format(k)
    ...
    def misspellwork(self, n: str, k: int) -> str:
        ...
        steps += (
            "P = n!/(a!b!c!...z!) - 1, where n is the number of letters"
            + " in the word and a,b,c,..,z are the number of times each letter"
            + " occurs \n"
        )
        steps += (
            "A word with no repeats will have P=n!-1 (All the variables"
            + " in the denominater are either 0! or 1! which results in the"
            + " denominator =1)\n"
        )
        steps += (
            "Ex. cake has P=4!-1 which is 24-1=23, we subtract 1 at the"
            + " end for the correct spelling of cake."
        )
        steps += (
            "Ex with repeats: success P=4!/(2!1!3!1!) - 1, the length is"
            + " 7 and there are (2c,1e,3s and 1u)\n"
        )
        steps += "success P=5040/(12) - 1 = 420-1 = 419\n"
        steps += (
            "For permutations or misspellings of a word shorter than the"
            + " word recall permutation formula:\n"
        )
        steps += (
            "m!/((m-k)!)(k!) where m is the number of unique letters in"
            + " the word and k is the target length.\n"
        )
        return steps
